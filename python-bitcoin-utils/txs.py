...
def get_standard_ct(
    ...
    timelock,
    locked: bool,
    tx_locktime_hex=None,
) -> Transaction:
    if l:
        tx_out0 = TxOutput(
            int(val_l - fee / 2),
            scripts.get_script_lightning_locked(id_l, id_r, hashed_secret, timelock),
        )  # output to l
        tx_out1 = TxOutput(int(val_r - fee / 2), id_r.p2pkh)  # output to r
    else:
        tx_out0 = TxOutput(
            int(val_r - fee / 2),
            scripts.get_script_lightning_locked(id_r, id_l, hashed_secret, timelock),
        )  # output to r
        tx_out1 = TxOutput(int(val_l - fee / 2), id_l.p2pkh)  # output to l

    if locked:
        tx = Transaction([tx_in], [tx_out0, tx_out1], tx_locktime_hex or "f0b78865")
    else:
...
def get_ALBA_ct(
    ...
    if l:  # ct_prover, has id and secret_V in opreturn output
        ...
        tx_out1 = TxOutput(int(val_r - fee / 2), id_r.p2pkh)  # output to r
        op_return_script_id = Script(
            ["OP_RETURN", "OP_1"]
        )  # omit id for checking valid latest state
        ...
        if locked:
            tx = Transaction(
                [tx_in], [tx_out0, tx_out1, tx_out2], tx_locktime_hex or "f0b78865"
            )  # timelock is Sun Dec 03 2023 12:53:20 GMT+0000 -> I had to flip bytes: UNIX timestamp is 1701608000 -> 656C7A40 -> 407a6c65
...
def get_standard_ct_spend(
    tx_in: TxInput, payee: Id, script_ct: Script, val: int, fee: int
) -> Transaction:
...
def get_standard_ct_punish(
    tx_in: TxInput, payee: Id, script_ct: Script, secret, val: int, fee: int
) -> Transaction:
    tx_out = TxOutput(val - fee, payee.p2pkh)
    tx = Transaction([tx_in], [tx_out])
...
def get_gen_split_tx(
    tx_in: TxInput, id_l: Id, id_r: Id, script: Script, val_l: int, val_r, fee: int
) -> Transaction:
...
def get_htlc_ct(
    ...
    if l:
        ...
        tx_out1 = TxOutput(
            int(val_l - fee / 2),
            scripts.get_htlc_script(
                id_l, id_r, hashed_secret, hashed_secret2, timelock
            ),
        )  # HTLC output
